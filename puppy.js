...
function pre() {
  ...
  for (var i=0; i<puppyLength; i++) {
    ...
    // internal components
    if (func[0].slice(0,2) === 'p-') returned = puppy.components.internal[func[0].substr(2)].apply(this, func.slice(1));
    // user/developer-defined components
    else returned = puppy.components[func[0]].apply(this, func.slice(1));
